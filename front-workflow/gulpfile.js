...
;(function (gulp, gulpLoadPlugins) {
    ...
    gulp.task('jshint', function () {
        return gulp.src([
            ...
            .pipe($.notify({
                message: '<%= options.date %> tasker jshint has verified: <%= file.relative %>',
                templateOptions: {
                    date: new Date()
                }
            }));
    ...
    gulp.task('jsonlint', function () {
        return gulp.src([
            ...
            .pipe($.notify({
                message: '<%= options.date %> tasker jsonlint has verified: <%= file.relative %>',
                templateOptions: {
                    date: new Date()
                }
            }));
    ...
    gulp.task('images', function () {
        return gulp.src([
                ...
                .pipe($.size())
                .pipe($.notify({
                    message: '<%= options.date %> tasker imagemin has compressed: <%= file.relative %>',
                    templateOptions: {
                        date: new Date()
                    }
                }));
    ...
    gulp.task('styles:dev', function () {
        return gulp.src(_.app + '/styles/less/main.less')
            .pipe($.plumber())
            .pipe($.less())
            .pipe($.connect.reload())
            ...
            .pipe($.size())
            .pipe($.notify({
                message: '<%= options.date %> tasker styles:dev has compiled: "<%= file.relative %>" file.',
                templateOptions: {
                    date: new Date()
                }
            }));
    ...
    gulp.task('scripts:dev', ['jshint'], function () {
        return gulp.src([
            ...
            .pipe($.size())
            .pipe($.notify({
                message: '<%= options.date %> tasker scripts:dev has updated: <%= file.relative %>',
                templateOptions: {
                    date: new Date()
                }
            }));
    ...
    gulp.task('html:dev', function () {
        return gulp.src(_.app + '/**/*.{htm,html,shtml,txt}')
            ...
            .pipe($.size())
            .pipe($.notify({
                message: '<%= options.date %> tasker html:dev has updated: <%= file.relative %>',
                templateOptions: {
                    date: new Date()
                }
            }));
    ...
    gulp.task('watch', function () {
        ...
        gulp.watch(_.app + '/styles/less/main.less', ['styles:dev']);
