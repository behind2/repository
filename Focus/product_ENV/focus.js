...
define(['jquery'], function ($) {
    ...
    Focus.prototype.bindDOM = function () {
        ...
        var displayContentTab = function (index, flag) {
            ...
            if ( _this.type === 'tab' ) {
                ...
                displayContentTab = function (index, flag) {
                    var dtd;
                    // 切换前
                    dtd = $.proxy(_this.switchBefore, _this)(_this.defShowTabIdx, _this.tabContentTags.get(_this.defShowTabIdx));
                    // 动画
                    var tabAnimation = function () {
                    ...
                    if ( typeof dtd === 'object' && typeof dtd.done === 'function' ) {// 有强烈时序的要求
                        dtd
                        .done(tabAnimation)
                        .done(after);
                    } else {// 没有就正常调用
                ...
                displayContentTab = function (index, flag) {
                    ...
                    if ( typeof dtd === 'object' && typeof dtd.done === 'function' ) {
                        dtd
                        .done(slideAnimation)
                        .done(after);
                    } else {
        ...
        // 停止自动播放
        this.ctrl.stopAutoToggle = function () {
            stopAutoToggle.apply(_this, arguments);
            _this.isAuto = false;
        };
        // 自动播放
        this.ctrl.autoToggle = function () {
            _this.isAuto = true;
            autoToggle.apply(_this, arguments);
        };
